...
**Now that we have an idea of what we are working towards, any questions?**
...
<--- edit to be discussion of overall depth--->
...
How many reads were generated per sample? We can use the `summarize_all` function from dplyr to determine the number of reads or "depth" per sample.

```{r SummarizeReads, eval=TRUE}
count_table %>% summarize_all(funs(sum))
```
...
R and RStudio have a strong community component so, as mentioned in Computational Foundations, if you are getting an error or wondering how to make a command work or how to perform a specific task, there is likely already a solution out there. Remember that search engines are your friend. 
