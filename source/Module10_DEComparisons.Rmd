...
```{r SetsResult}
results_deficient_vs_standard = results(dds_fitted, name = 'condition_deficient_vs_standard')
head(results_deficient_vs_standard)
```
...
2. `log2FoldChange` is the log2 transformed ratio of the expression of the numerator group (first group) over the denominator group (second group after "vs"). Note that in our comparison, the `log2FoldChange` column compares the expression of `deficient` over the denominator group, `standard`. If the value is positive, that means the expression of that gene is greater across the `deficient` samples than across the `standard` samples. If the value is negative, that means the expression of that gene is greater across the `deficient` samples.
...
So an alternative way to test the same contrast as above (i.e. `standard` / `deficient`) is:

```{r SetsResultsAlt}
alt_results_deficient_vs_standard = results(dds_fitted, contrast = c('condition', 'deficient', 'standard'))
head(alt_results_deficient_vs_standard)
```
